...
export default function WhyChooseUs() {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden py-24 px-6 bg-gradient-to-b from-[#04113A] via-[#061547] to-[#071A52]"
    >
      {/* Glow Effects */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="w-24 h-1 bg-accent mx-auto rounded-full mb-5" aria-hidden="true" />
          <p className="text-white/70 text-base md:text-xl leading-relaxed max-w-3xl mx-auto">
          ...
          {features.map((feature, i) => {
            ...
            return (
              <motion.li
                ...
                className="relative overflow-hidden rounded-[30px] border border-white/10 bg-white/5 backdrop-blur-xl p-8 group shadow-[0_10px_30px_rgba(0,0,0,0.25)] hover:shadow-[0_20px_50px_rgba(255,122,26,0.20)] transition-all duration-500"
              >
                {/* Glow */}
                <div
                  className="absolute top-0 right-0 w-40 h-40 bg-accent/10 blur-3xl rounded-full group-hover:bg-accent/20 transition-colors duration-500"
                  aria-hidden="true"
                />

                {/* Icon */}
                <div className="relative z-10 w-20 h-20 rounded-2xl bg-accent/10 border border-accent/20 flex items-center justify-center mb-6 group-hover:bg-accent transition-colors duration-300">
                  <Icon
                    className="w-10 h-10 text-accent group-hover:text-white transition-colors duration-300"
                    aria-hidden="true"
                  />
                </div>

                {/* Title */}
                <h3 className="relative z-10 text-2xl font-display font-bold text-white mb-4 leading-snug">
                  {feature.title}
                </h3>
